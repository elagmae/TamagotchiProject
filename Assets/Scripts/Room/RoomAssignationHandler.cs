...
public class RoomAssignationHandler : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            var state = RoomManager.CurrentRoomData.AnimalStates[i];
            state.Value = _states[i].fillAmount * 100f;
            RoomManager.CurrentRoomData.AnimalStates[i] = state;
        }

        print(RoomManager.CurrentRoomData.AnimalStates[1].Value);
    }
    ...
    public async void SaveRoom()
    {
        ...
        {
            {"id", RoomManager.CurrentRoomId },
            {"file", RoomManager.CurrentRoomData}
        };
